...
export const updateActivity = async (req, res) => {
    try {
        ...
        const activity = await Activity.findByIdAndUpdate(req.params.id, { activityType, duration, date, notes }, { new: true, runValidators: true }).populate('userId', 'username email');
        if (!activity) {
            res.status(404).json({ success: false, error: 'Activity not found' });
            return;
        }
